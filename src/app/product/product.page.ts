...
export class ProductPage {
	...
	getVideo(){
		if(this.product.meta_data){
            for(let item in this.product.meta_data){
                if(this.product.meta_data[item].key == '_ywcfav_video'){
                    this.videos = this.product.meta_data[item].value ;
					//this.videoss = Array.of(this.videos);
					for(let content in this.videos){
					this.videoss = this.videos[content].content;
					}
                }
            }
        }
	...
	getUrlVideo(){
		...
		this.http.get('https://hmkalindojava.com/wp-json/wp/v2/media/'+this.videoss+'?consumer_key=ck_36e98826cb3cf9e367436c839f761ce3200dc838&consumer_secret=cs_4256b907ed6d2c974c93fb82c6fe68578e579853').subscribe(data =>{
            this.urlvideo = data;
            this.urlvideo = Array.of(this.urlvideo);  
        });
